...
const Dashboard = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`https://localhost:5000/api/v1/emergency`, {
          method: "GET",
  ...
  const getChats = async (emerge) => {
    try {
      const res = await fetch(`https://localhost:5000/api/v1/chats/${auth?.user?._id}/emerg/${emerge}`, {
        method: "GET",
        headers: { 'Content-type': "application/json" }
      });
      if (res.status === 200) {
  ...
  const addChat = async (receiverId, emergId) => {
    try {
      ...
      console.log(payload)
      const res = await fetch("https://localhost:5000/api/v1/chats", {
        method: "POST",
        ...
        <div className='features_wrapper' style={{ marginTop: '-50px' }}>
          <div className="row">
            <div className="col-12 text-center">
              <p className="features_subtitle">Latest Human Emergency Alert !</p>
              <h2 className="features_title">Human Emergency Data</h2>
            </div>
          </div>
        </div>
