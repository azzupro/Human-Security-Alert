...
const Dashboard = (props) => {
  ...
  const getAllIncident = async () => {
    try {
      const { data } = await axios.get('https://localhost:5000/api/v1/incidents')
        ...
        <div className='features_wrapper' style={{ marginTop: '-50px' }}>
          <div className="row">
            <div className="col-12 text-center">
              <p className="features_subtitle">Latest Human Incident Reported !</p>
              <h2 className="features_title">Human Incident Data</h2>
            </div>
          </div>
        </div>
